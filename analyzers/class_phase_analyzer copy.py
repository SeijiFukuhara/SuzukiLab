...
#? 関数ファイルから必要な関数をインポート
from function_calculation import offset, plot_phase,approximation_phase, shift_2d_array, make_k_dict

import streamlit as st
@st.cache_data
def make_phase_dict(img_phase_array, convolve_size_temp, offset_area_slice, x_axis_pix_phase, width_phase, height_phase, n_apr_pix, gaussian_additive_term, d_micro_to_pix_temp, x_adjust):
    #* original
    phase_original_dict = {"original": img_phase_array}
    #* offset
    img_phase_array_offset = offset(img_phase_array, convolve_size_temp, offset_area_slice, convolve = False)
    phase_offset_dict = {"offset": img_phase_array_offset}
    #* offset and convolve
    img_phase_array_offset_convolve = offset(img_phase_array, convolve_size_temp, offset_area_slice, convolve = True)
...
class PhaseAnalyzer:
    ...
    def __init__(self, csv_file, d_micro_to_pix_temp, convolve_size_temp,Nx, l, h0,  n_apr_pix, offset_area_slice, gaussian_additive_term, debug_k_pix, microm_or_pix, x_adjust):
        # self.path = csv_file
        ...
        # self.offset_area_slice = offset_area_slice
        ...
        #* csvファイル読み込み
        df = pd.read_csv(csv_file, encoding="cp932",index_col = None, header = None, engine = 'python')
        ...
        #* x軸の作成（pix）
        self.x_axis_pix_phase = np.arange(-Nx, -Nx + width_phase)
